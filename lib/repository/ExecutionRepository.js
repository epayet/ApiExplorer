...
function getPath(parameters, path, parametersDefinition) {
    addParametersToPath("id");
    addParametersToPath("query");
    return path;

    function addParametersToPath(parameterType) {
        var i = 0;
        for(var key in parameters) {
            if(parameters[key] !== "") {
                if (parameterType == "query" && (isParameterType(key, parameterType, parametersDefinition))) {
                    path = addQueryParameter(path, key, parameters[key], i);
                    i++;
                } else if (parameterType == "id" && (isParameterType(key, parameterType, parametersDefinition))) {
                    path = path.replace("{id}", parameters[key]);
                    break;
                }
            }
        }
